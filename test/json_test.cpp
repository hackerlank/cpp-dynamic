...
//
// MSVC 2013 does not support unicode/raw string literals
//
...
{
    json::serialization_opts on, off;
...
TEST(Json, JavascriptSafe)
{
    ...
    EXPECT_EQ(toJson(badDouble), to<std::string>(badDouble));
    json::serialization_opts opts;
    opts.javascript_safe = true;
    EXPECT_ANY_THROW(json::serialize(badDouble, opts));

    auto okDouble = 1ll << 63ll;
    dynamic okDyn = okDouble;
    EXPECT_EQ(toJson(okDouble), to<std::string>(okDouble));
}
...
TEST(Json, JsonNonAsciiEncoding)
{
    ...
    // simple tests
    EXPECT_EQ(json::serialize("\x1f", opts), R"("\u001f")");
    EXPECT_EQ(json::serialize("\xc2\xa2", opts), R"("\u00a2")");
    EXPECT_EQ(json::serialize("\xe2\x82\xac", opts), R"("\u20ac")");

#if __GNUC_PREREQ(4, 7)
    // multiple unicode encodings
    EXPECT_EQ(
        json::serialize("\x1f\xe2\x82\xac", opts),
        R"("\u001f\u20ac")");
    EXPECT_EQ(
        json::serialize("\x1f\xc2\xa2\xe2\x82\xac", opts),
        R"("\u001f\u00a2\u20ac")");
    EXPECT_EQ(
        json::serialize("\xc2\x80\xef\xbf\xbf", opts),
        R"("\u0080\uffff")");
    EXPECT_EQ(
        json::serialize("\xe0\xa0\x80\xdf\xbf", opts),
        R"("\u0800\u07ff")");

    // first possible sequence of a certain length
    EXPECT_EQ(json::serialize("\xc2\x80", opts), R"("\u0080")");
    EXPECT_EQ(json::serialize("\xe0\xa0\x80", opts), R"("\u0800")");

    // last possible sequence of a certain length
    EXPECT_EQ(json::serialize("\xdf\xbf", opts), R"("\u07ff")");
    EXPECT_EQ(json::serialize("\xef\xbf\xbf", opts), R"("\uffff")");

    // other boundary conditions
    EXPECT_EQ(json::serialize("\xed\x9f\xbf", opts), R"("\ud7ff")");
    EXPECT_EQ(json::serialize("\xee\x80\x80", opts), R"("\ue000")");
    EXPECT_EQ(json::serialize("\xef\xbf\xbd", opts), R"("\ufffd")");
#endif

    // incomplete sequences
    EXPECT_ANY_THROW(json::serialize("a\xed\x9f", opts));
    EXPECT_ANY_THROW(json::serialize("b\xee\x80", opts));
    EXPECT_ANY_THROW(json::serialize("c\xef\xbf", opts));

    // impossible bytes
    EXPECT_ANY_THROW(json::serialize("\xfe", opts));
    EXPECT_ANY_THROW(json::serialize("\xff", opts));

    // Sample overlong sequences
    EXPECT_ANY_THROW(json::serialize("\xc0\xaf", opts));
    EXPECT_ANY_THROW(json::serialize("\xe0\x80\xaf", opts));

    // Maximum overlong sequences
    EXPECT_ANY_THROW(json::serialize("\xc1\xbf", opts));
    EXPECT_ANY_THROW(json::serialize("\x30\x9f\xbf", opts));

    // illegal code positions
    EXPECT_ANY_THROW(json::serialize("\xed\xa0\x80", opts));
    EXPECT_ANY_THROW(json::serialize("\xed\xbf\xbf", opts));

    // Overlong representation of NUL character
    EXPECT_ANY_THROW(json::serialize("\xc0\x80", opts));
    EXPECT_ANY_THROW(json::serialize("\xe0\x80\x80", opts));

    // Longer than 3 byte encodings
    EXPECT_ANY_THROW(json::serialize("\xf4\x8f\xbf\xbf", opts));
    EXPECT_ANY_THROW(json::serialize("\xed\xaf\xbf\xed\xbf\xbf", opts));
}
...
TEST(Json, UTF8Retention)
{
    ...
    std::string input = "\u2665";
    std::string jsonInput = toJson(input);
    std::string output = parseJson(jsonInput).asString();
    std::string jsonOutput = toJson(output);
...
TEST(Json, UTF8EncodeNonAsciiRetention)
{
    ...
    std::string input = "\u2665";
    std::string jsonInput = json::serialize(input, opts);
    std::string output = parseJson(jsonInput).asString();
    std::string jsonOutput = json::serialize(output, opts);
    ...
    EXPECT_ANY_THROW(json::serialize("a\xe0\xa0\x80z\xc0\x80", opts));
    EXPECT_ANY_THROW(json::serialize("a\xe0\xa0\x80z\xe0\x80\x80", opts));
}
...
TEST(Json, UTF8Validation)
{
    ...
    EXPECT_EQ(json::serialize("a\xc2\x80z", opts), "\"a\xc2\x80z\"");
    EXPECT_EQ(
        json::serialize("a\xe0\xa0\x80z", opts),
        "\"a\xe0\xa0\x80z\"");
    EXPECT_EQ(
        json::serialize("a\xe0\xa0\x80m\xc2\x80z", opts),
    ...
    EXPECT_EQ(json::serialize("a\xe0\xa0\x80z\xc0\x80", opts),
        "\"a\xe0\xa0\x80z\ufffd\ufffd\"");
    EXPECT_EQ(json::serialize("a\xe0\xa0\x80z\xc0\x80\x80", opts),
        "\"a\xe0\xa0\x80z\ufffd\ufffd\ufffd\"");
    EXPECT_EQ(json::serialize("z\xc0\x80z\xe0\xa0\x80", opts),
        "\"z\ufffd\ufffdz\xe0\xa0\x80\"");
...
TEST(Json, SortKeys)
{
    json::serialization_opts opts_on, opts_off;
    ...
    EXPECT_EQ(value, parseJson(json::serialize(value, opts_on)));
    EXPECT_EQ(value, parseJson(json::serialize(value, opts_off)));

    EXPECT_EQ(sorted_keys, json::serialize(value, opts_on));
}
